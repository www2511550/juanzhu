...
<article>
    ...
    <div class="infos">
        <div class="newsview">
            <h1 class="news_title">{{$data->title}}</h1>
            ...
            <div class="news_infos">
                @if(isset($data->content['dapei']))
                    @foreach($data->content['dapei'] as $vo)

                        <p>{{ $vo['intro'] }}</p>
                        <p><img class="lazy" src="{{ asset('loading.png') }}" data-original="{{ $vo['cover'] }}" alt="{{ $vo['title'] }}"></p>
                        <p style="text-align: center;">
                            <a data-type="0" biz-itemid="{{$vo['goods_id']}}" data-tmpl="192x40" data-tmplid="623" data-rd="2" data-style="2" data-border="1" href="{{$vo['href']}}">点我购买</a>
                        </p>
                    @endforeach
                @else
                    @foreach($data->content as $vo)
                        ...
                        <p style="text-align: center;">
                            ...
                            @else
                                <a data-type="0" biz-itemid="{{$vo['goods_id']}}" data-tmpl="192x40" data-tmplid="623" data-rd="2" data-style="2" data-border="1" href="{{$vo['to_url']}}">点我购买</a>
                            @endif
                        </p>
                    @endforeach
                @endif
        ...
        <div class="share"> </div>
        <div class="nextinfo">
            @if($prePage->id)<p>上一篇：<a href="{{ route('dp.detail', ['id'=>$prePage->id]) }}">{{ $prePage->title }}</a></p>@endif
            @if($nextPage->id)<p>下一篇：<a href="{{ route('dp.detail', ['id'=>$nextPage->id]) }}">{{ $nextPage->title }}</a></p>@endif
        </div>
