    ...
    <div class="infos">
        <div class="newsview">
            ...
            <div class="news_author" style="margin-bottom: 20px;">
                ...
                <span class="au02">{{date('Y-m-d', strtotime($info->created_at))}}</span><span class="au03">共<b><script
                                src=""></script>{{$info->browse_num}}</b>人围观</span>
            </div>
        ...
        <div class="share"> </div>
        <div class="nextinfo">
            @if($prePage->id)<p>上一篇：<a href="{{ route('b.detail', ['id'=>$prePage->id]) }}">{{ $prePage->title }}</a></p>@endif
            @if($nextPage->id)<p>下一篇：<a href="{{ route('b.detail', ['id'=>$nextPage->id]) }}">{{ $nextPage->title }}</a></p>@endif
        </div>
